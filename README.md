...
- ⚖️ **القانون والدفاع والحقوق** (Law, Defense and Rights Advocacy)
- 🎓 **التعليم والتمكين** (Education and Empowerment)
...
> **ملاحظة:** ملف اللوغو الحالي (`static/img/logo.jpg`) يحتوي على نص "جمعية اجتماعية ثقافية"
> مضمَّن داخل الصورة. يُفضّل تجديد اللوغو ليعكس المجالات الثلاثة الجديدة.
...
النظام يدعم تشغيل **بيئتين مستقلتين في نفس الوقت**:

| البيئة | الغرض | المنفذ | قاعدة البيانات |
|--------|------|------|----------------|
| 🟦 **الإنتاج** | التوثيق الفعلي للناجين | 8000 | `db.sqlite3` |
| 🟨 **التدريب** | تدريب المتطوّعين | 8001 | `db.training.sqlite3` |

**البيانات منفصلة فصلاً تاماً:** قواعد بيانات مختلفة، ملفات ميديا مختلفة،
جلسات مختلفة، سجلات تدقيق مختلفة. لا يمكن للمتدرّب أن يؤثر بأي شكل على
بيانات التوثيق الفعلية.

### 🚀 تشغيل الإنتاج (التوثيق الفعلي)
...
### 🎓 تشغيل بيئة التدريب (للمتطوّعين)

**Linux / macOS:**
```bash
./run_training.sh    # المنفذ 8001 - قاعدة منفصلة
```

**Make:**
```bash
make train             # تشغيل
make train-seed        # بذر 3 ناجين تجريبيين بمستويات مختلفة
make train-reset       # مسح كل بيانات التدريب والبدء من جديد
```

**ميزات بيئة التدريب:**
- 🟨 شريط أصفر مخطّط بارز يُذكّر المتدرّب أنه في بيئة تجريبية
- 🎨 navbar بلون مختلف (بنّي/برتقالي) للتمييز البصري
- 🔄 إمكانية مسح القاعدة كاملاً وإعادة البدء بأمر واحد
- 📝 بيانات تجريبية جاهزة (ملف مكتمل + جزئي + مسودة فارغة)
- 🔒 الأوامر الحساسة محمية بفحص مزدوج (لن تعمل على الإنتاج بالخطأ)

### يمكن تشغيلهما بالتوازي
```bash
# Terminal 1 - الإنتاج
./run.sh
